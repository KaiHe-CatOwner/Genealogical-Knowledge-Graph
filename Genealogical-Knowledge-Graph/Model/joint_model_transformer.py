...
parser.add_argument('--Task_list', default="[12,3,4,5]", type=str, help="[12,3,4,5]")
parser.add_argument('--Loss_weight_list', default="[0.6, 0.2, 0.2, 0.5]", type=str, help="keep same len with Task_list")
...
parser.add_argument('--Weight_Lambda_3', default=10, type=float)
# Task 4
...
parser.add_argument('--Weight_Lambda_5', default=1, type=float)
...
file_performance_total = '../result/detail_performance/Transformer/total_performance.txt'
file_performance = '../result/detail_performance/Transformer/performance_' + str(sys.argv[1:]) + '.txt'
file_result = '../result/detail_results/Transformer/result_' + str(sys.argv[1:]) + '.txt'
file_model_save = "../result/save_model/Transformer/" + str(sys.argv[1:])
file_traning_performerance ='../result/detail_training/Transformer/training_'+str(sys.argv[1:])+'.txt'
...
class My_Common_Encoder(nn.Module):
    def __init__(self, embedding_dim, TOEKNS):
        ...
        self.transformer_encoder = nn.TransformerEncoderLayer(d_model=self.embedding_dim, nhead=args.Head_Num)
    ...
    def forward(self, x):
        ...
        out_put = self.transformer_encoder(embedded)
        return out_put
