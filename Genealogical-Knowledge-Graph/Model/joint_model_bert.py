...
import sys
from joint_prepare_name_location import make_task5_final_list, get_task_5_predict_to_gold, prepare_test_batch_data, \
    prepare_train_batch_data
...
parser.add_argument('--bert_model', default="tiny", type=str, help="tiny, base")
parser.add_argument('--EPOCH', default=300, type=int)
parser.add_argument('--NUM_FOLD', default=1, type=int)
parser.add_argument('--Strat_Fold', default=0, type=int)
...
parser.add_argument('--MIN_EPOCH', default=70, type=int)
parser.add_argument('--Start_Decay', default=70, type=int)
parser.add_argument('--Decay_Cycle', default=10, type=int)
parser.add_argument('--GPU', default="0", type=str)
# Task 0
parser.add_argument('--LR_common_encoder', default=3e-5, type=float)
...
parser.add_argument('--Simply_num', default=0, type=int, help="# 0 will contain all kinship /10 / 50")
parser.add_argument('--Augment_Data', default=None, type=str,
                    help="None, or file name: BIES_bert_base_numberic_agument_data_0.5_200.json")
...
parser.add_argument('--Weight_Lambda_3', default=1, type=float)
# Task 4
...
parser.add_argument('--LR_5', default=1e-4, type=float)
...
print()
# when change model, need change tokenizer in data process / hidden_size = 128
if args.bert_model == "tiny":
    args.model_path = "../../../Data/embedding/uncased_L-2_H-128_A-2"
    args.Word_embedding_size = 128
    args.Hidden_Size_Common_Encoder = args.Word_embedding_size
elif args.bert_model == "base":
    args.model_path = "../../../Data/embedding/uncased_L-12_H-768_A-12"
...
file_performance_total = '../result/detail_performance/Bert/total_performance.txt'
file_performance = '../result/detail_performance/Bert/performance_' + str(sys.argv[1:]) + '.txt'
file_result = '../result/detail_results/Bert/result_' + str(sys.argv[1:]) + '.txt'
file_model_save = "../result/save_model/Bert/" + str(sys.argv[1:])
file_traning_performerance = '../result/detail_training/Bert/training_' + str(sys.argv[1:]) + '.txt'


# Common encoder
class My_Common_Encoder(nn.Module):
    def __init__(self, embedding_dim, TOEKNS_stoi, bert):
        super(My_Common_Encoder, self).__init__()
        # self.to(device)
        self.TOEKNS_stoi = TOEKNS_stoi
    ...
    def forward(self, batch_inputs):
        ...
        common_embedding = self.bert(tokens_tensor)[0]
...
class My_Model_5(nn.Module):
    ...
    def forward(self, common_embedding, batch_data, tokenizer):
        Batch_task5_pair, Batch_task5_common_encoder, batch_gold_list, split_name_count_total = \
            prepare_train_batch_data(common_embedding, self.TOEKNS_5, self.TAGS_5, self.TAGS_5_NEW, batch_data,
                                     bert_flag=True, tokenizer=tokenizer)
    ...
    def only_for_task5_valid(my_model_5, common_embedding, batch_data,
                             TOEKNS_5, TAGS_5, TAGS_5_NEW, tokenizer):
        Batch_task5_pair, Batch_task5_common_encoder, batch_gold_list, split_name_count_total = \
            prepare_train_batch_data(common_embedding, TOEKNS_5, TAGS_5, TAGS_5_NEW, batch_data, bert_flag=True, tokenizer=tokenizer)
    ...
    def my_valid(my_model_5, common_embbeding, batch_data,
                 TOEKNS_5, TAGS_5, TAGS_5_NEW, TAGS_12, entity_extracted_in_12, tokenizer):

        Batch_task5_pair, Batch_task5_common_encoder, batch_gold_list, split_name_count_total = \
            prepare_test_batch_data(common_embbeding, TOEKNS_5, TAGS_5, TAGS_5_NEW, TAGS_12, entity_extracted_in_12,
                                    batch_data, args.Improve_Flag, bert_flag=True, tokenizer=tokenizer)
...
class Train_joint_model():
    def __init__(self, dic_init, TAGS_5_NEW, train_test_iterator_dic, TOEKNS_TAGS_dic, bert, tokenizer):
        ...
        self.common_encoder = My_Common_Encoder(args.Word_embedding_size, self.TOEKNS_TAGS_dic[0][0], self.bert).to(device)
        ...
        if 5 in eval(args.Task_list):
            self.model_5 = My_Model_5(*dic_init[5], self.TOEKNS_TAGS_dic[5][0], self.TOEKNS_TAGS_dic[5][1],
                                      self.TAGS_5_NEW).to(device)
        ...
        if args.Continue_training:
            ...
            self.optimizer_0.load_state_dict(checkpoint['optimizer_0'])

            if 12 in eval(args.Task_list):
                self.model_12.load_state_dict(checkpoint['model_12_state_dict'])
                self.optimizer_12.load_state_dict(checkpoint['optimizer_12'])
            if 3 in eval(args.Task_list):
            ...
            if 4 in eval(args.Task_list):
                ...
                self.optimizer_4.load_state_dict(checkpoint['optimizer_4'])
            if 5 in eval(args.Task_list):
                self.model_5.load_state_dict(checkpoint['model_5_state_dict'])
                self.optimizer_5.load_state_dict(checkpoint['optimizer_5'])
    ...
    def save_model(self, epoch):
        ...
        self.model_state_dic['optimizer_0'] = self.optimizer_0.state_dict()

        if 12 in eval(args.Task_list):
            self.model_state_dic['model_12_state_dict'] = self.model_12.state_dict()
            self.model_state_dic['optimizer_12'] = self.optimizer_12.state_dict()
        if 3 in eval(args.Task_list):
            self.model_state_dic['model_3_state_dict'] = self.model_3.state_dict()
            self.model_state_dic['optimizer_3'] = self.optimizer_3.state_dict()
        if 4 in eval(args.Task_list):
            self.model_state_dic['model_4_state_dict'] = self.model_4.state_dict()
            self.model_state_dic['optimizer_4'] = self.optimizer_4.state_dict()
        if 5 in eval(args.Task_list):
            self.model_state_dic['model_5_state_dict'] = self.model_5.state_dict()
            self.model_state_dic['optimizer_5'] = self.optimizer_5.state_dict()
    ...
    def joint_train(self, tasks_batch_data_dic):
        ...
        if 4 in eval(args.Task_list):
            ...
            thread_4.start()
        if 5 in eval(args.Task_list):
            thread_5 = MyThread(self.model_5.forward, args=[common_embedding, tasks_batch_data_dic[5], self.tokenizer])
            thread_5.start()
        ...
        if 12 in eval(args.Task_list):
            ...
            criterion_12 = get_loss(args.IF_Entity_Weight, args.IF_Task12_Adap_Loss, args.Weight_Lambda_12,
                                    tasks_batch_data_dic[12].tags, self.TOEKNS_TAGS_dic, self.TAGS_5_NEW,
                                    entity_flag=True)
        ...
        if 3 in eval(args.Task_list):
            ...
            criterion_3 = get_loss(args.IF_NamePropagation_Weight, args.IF_Task3_Adap_Loss, args.Weight_Lambda_3,
                                   tasks_batch_data_dic[3].tags, self.TOEKNS_TAGS_dic, self.TAGS_5_NEW,
        ...
        if 4 in eval(args.Task_list):
            ...
            criterion_4 = get_loss(args.IF_Parentheses_Weight, args.IF_Task4_Adap_Loss, args.Weight_Lambda_4,
                                   tasks_batch_data_dic[4].tags, self.TOEKNS_TAGS_dic, self.TAGS_5_NEW,
        ...
        if 5 in eval(args.Task_list):
            ...
            if len(prediction_5) == 0:
                ...
                task_5_predict_gold_batch = get_task_5_predict_to_gold(task_5_gold_batch, task_5_predict_batch,
                                                                       self.TAGS_5_NEW)
    ...
    def train_model(self, fold_num):
        ...
        for epoch in range(self.current_epoch, args.EPOCH):
                ...
                for batch_item in zip(*need_zip_list):
                    ...
                    joint_loss, prediction_dic, loss_person_location, split_name_count_batch = self.joint_train(
                        tasks_batch_data_dic)
                    ...
                    batch_TP_FN_FP_dic, _, _, _ = get_batch_TP_FN_FP(prediction_dic, tasks_batch_data_dic,
                                                                     self.TOEKNS_TAGS_dic)
                ...
                # # tasks_result_dic[i] = [task12_micro_result, task12_macro_result, task12_TP_FN_FP, task12_each_PRF]
                ...
                with SummaryWriter('runs/My_model') as w:
                    w.add_scalar('Loss', total_loss / step, global_step=epoch)

                if 5 in eval(args.Task_list):
                    print(
                        "                                                                                         task 5: Loss: %.3f, P: %.3f, R: %.3f, F: %.3f" %
                ...
                with open(file_traning_performerance, write_flag) as f:
                    f.writelines(
                        'Data_ID: %2d, Fold_num: %2d,  Epoch: %2d, Train Loss: %.3f, P: %.3f, R: %.3f, F: %.3f \n' %
                        (
                            int(args.Data_ID), fold_num, epoch, total_loss / step, epoch_P_macro, epoch_R_macro,
                            epoch_F_macro))
        ...
        if 5 in eval(args.Task_list):
            if 12 in eval(args.Task_list):
                thread_5 = MyThread(My_Model_5.my_valid, args=[self.model_5, common_embbeding, tasks_batch_data_dic[5],
                                                               ...
                                                               self.TAGS_5_NEW, self.TOEKNS_TAGS_dic[12][1],
                                                               torch.argmax(predictions_12, 1), self.tokenizer])
            else:
                thread_5 = MyThread(My_Model_5.only_for_task5_valid,
                                    args=[self.model_5, common_embbeding, tasks_batch_data_dic[5],
                                          self.TOEKNS_TAGS_dic[5][0], self.TOEKNS_TAGS_dic[5][1], self.TAGS_5_NEW, self.tokenizer])
    ...
    def valid_model(self, fold_num, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_num, item in enumerate(zip(*need_task_zip_list)):
                ...
                joint_loss, prediction_dic, loss_person_location, split_name_count_batch_valid = self.joint_valid(
                    tasks_batch_data_dic)
                split_name_count_total_valid += split_name_count_batch_valid

                batch_TP_FN_FP_dic, tokens_total, targets_total, predictions_total = get_batch_TP_FN_FP(prediction_dic,
                                                                                                        tasks_batch_data_dic,
                                                                                                        self.TOEKNS_TAGS_dic,
                                                                                                        valid_flag=True)
                for task in eval(args.Task_list):
                ...
                test_data_count = record_detail_result(file_result, tokens_total, targets_total, predictions_total,
                                                       self.TOEKNS_TAGS_dic, eval(args.Task_list), batch_num, fold_num,
                                                       test_data_count, bert_flag=True)

            dic_reslut = get_epoch_P_R_F(total_TP_FN_FP_dic)

            # dic_reslut[i] = [task12_micro_result, task12_macro_result, task12_TP_FN_FP , task12_each_PRF]
            for key, value in dic_reslut.items():
            ...
            if 5 in eval(args.Task_list):
                ...
                with open(file_traning_performerance, "a") as f:
                    ...
                    f.writelines(
                        'Data_ID: %2d, Fold_num: %2d,  Epoch: %2d, Train Loss: %.3f, P: %.3f, R: %.3f, F: %.3f \n' %
                        (
                            int(args.Data_ID), fold_num, epoch, total_loss / step, task5_epoch_P_macro,
                            task5_epoch_R_macro,
                            task5_epoch_F_macro))

            total_micro_result = [epoch_P_micro, epoch_R_micro, epoch_F_micro]
            total_macro_result = [epoch_P_macro, epoch_R_macro, epoch_F_macro]
            write_dic_performance(file_performance, total_micro_result, total_macro_result, dic_reslut,
                                  eval(args.Task_list), fold_num, epoch)
...
def get_epoch_P_R_F(total_TP_FN_FP_dic):
    ...
    if 12 in eval(args.Task_list):
        ...
        P_entity_macro, R_entity_macro, F_entity_macro, task12_each_PRF \
        ...
        task12_result_list = [task12_micro_result, task12_macro_result, task12_TP_FN_FP, task12_each_PRF]
    ...
    if 3 in eval(args.Task_list):
        ...
        P_name_propagation_macro, R_name_propagation_macro, F_name_propagation_macro, task3_each_PRF \
        ...
        task3_result_list = [task3_micro_result, task3_macro_result, task3_TP_FN_FP, task3_each_PRF]
    ...
    if 4 in eval(args.Task_list):
        ...
        P_parentheses_macro, R_parentheses_macro, F_parentheses_macro, task4_each_PRF \
        ...
        task4_result_list = [task4_micro_result, task4_macro_result, task4_TP_FN_FP, task4_each_PRF]
    ...
    if 5 in eval(args.Task_list):
        ...
        if total_TP_FN_FP_dic[5] == {'L-N': (0, 0, 0)}:
            ...
            P_Name_Location_macro, R_Name_Location_macro, F_Name_Location_macro, task5_each_PRF \
        ...
        task5_result_list = [task5_micro_result, task5_macro_result, task5_TP_FN_FP, task5_each_PRF]
...
def get_valid_performance(model_path):
    ...
    for i in range(args.Strat_Fold, args.NUM_FOLD):  # NUM_FOLD = 10
        print("==========================" + str(i) + "=================================================")
        ...
        establish_file(i, args.Simply_num, args.Data_ID, args.My_Test_data_flag, bert_model=args.bert_model)

        train_set_0, test_set_0, _, TAGS_0 = prepared_data(args.Data_ID,
        ...
        train_iterator_0, test_iterator_0, _, TAGS_0 = filter_content(
            train_set_0, test_set_0, _, TAGS_0, args.BATCH_SIZE, device,
            entity_only_flag=True)

        TOEKNS_TAGS_dic[0] = (tokenizer.convert_ids_to_tokens, TAGS_0)
        train_test_iterator_dic[0] = (train_iterator_0, test_iterator_0)

        if 12 in eval(args.Task_list):
            train_set_12, test_set_12, _, TAGS_12 = prepared_data(args.Data_ID,
                                                                  args.Word_embedding_size,
                                                                  pad_ID=tokenizer.convert_tokens_to_ids("<PAD>"),
                                                                  unk_ID=tokenizer.convert_tokens_to_ids("[UNK]"),
                                                                  bert_flag=True,
            ...
            train_iterator_12, test_iterator_12, _, TAGS_12 = filter_content(
                train_set_12, test_set_12, _, TAGS_12, args.BATCH_SIZE, device,
                entity_only_flag=True)

            dic_init[12] = args.Hidden_Size_Common_Encoder, len(TAGS_12.vocab)
            TOEKNS_TAGS_dic[12] = (tokenizer.convert_ids_to_tokens, TAGS_12)
            train_test_iterator_dic[12] = (train_iterator_12, test_iterator_12)

        if 3 in eval(args.Task_list):
            train_set_3, test_set_3, _, TAGS_3 = prepared_data(args.Data_ID,
                                                               args.Word_embedding_size,
                                                               pad_ID=tokenizer.convert_tokens_to_ids("<PAD>"),
                                                               unk_ID=tokenizer.convert_tokens_to_ids("[UNK]"),
                                                               bert_flag=True,
            ...
            train_iterator_3, test_iterator_3, _, TAGS_3 = filter_content(
                train_set_3, test_set_3, _, TAGS_3, args.BATCH_SIZE, device,
                name_propagation_flag=True)

            dic_init[3] = args.Hidden_Size_Common_Encoder, len(TAGS_3.vocab)
            TOEKNS_TAGS_dic[3] = (tokenizer.convert_ids_to_tokens, TAGS_3)
            train_test_iterator_dic[3] = (train_iterator_3, test_iterator_3)

        if 4 in eval(args.Task_list):
            train_set_4, test_set_4, _, TAGS_4 = prepared_data(args.Data_ID,
                                                               args.Word_embedding_size,
                                                               pad_ID=tokenizer.convert_tokens_to_ids("<PAD>"),
                                                               unk_ID=tokenizer.convert_tokens_to_ids("[UNK]"),
                                                               bert_flag=True,
            ...
            train_iterator_4, test_iterator_4, _, TAGS_3 = filter_content(
                train_set_4, test_set_4, _, TAGS_4, args.BATCH_SIZE, device,
                parentheses_flag=True)

            dic_init[4] = args.Hidden_Size_Common_Encoder, len(TAGS_4.vocab)
            TOEKNS_TAGS_dic[4] = (tokenizer.convert_ids_to_tokens, TAGS_4)
        ...
        if 5 in eval(args.Task_list):
            train_set_5, test_set_5, _, TAGS_5 = prepared_data(args.Data_ID,
                                                               args.Word_embedding_size,
                                                               pad_ID=tokenizer.convert_tokens_to_ids("<PAD>"),
                                                               unk_ID=tokenizer.convert_tokens_to_ids("[UNK]"),
                                                               bert_flag=True,
            ...
            train_iterator_5, test_iterator_5, _, TAGS_5 = filter_content(
                train_set_5, test_set_5, _, TAGS_5, args.BATCH_SIZE, device,
                person_location_flag=True)

            dic_init[5] = args.Hidden_Size_Common_Encoder * 3, len(TAGS_5_NEW)
            TOEKNS_TAGS_dic[5] = (tokenizer.convert_ids_to_tokens, TAGS_5)
            train_test_iterator_dic[5] = (train_iterator_5, test_iterator_5)

        joint_train = Train_joint_model(dic_init, TAGS_5_NEW, train_test_iterator_dic, TOEKNS_TAGS_dic, bert, tokenizer)
        stop_epoch, split_name_count_total_train = joint_train.train_model(i)
        args.Continue_training = False

        valid_joint_model = Valid_joint_model(dic_init, TAGS_5_NEW, train_test_iterator_dic, TOEKNS_TAGS_dic, bert, tokenizer)
    ...
    record_each_performance(file_performance_total, final_result_list_P_macro, final_result_list_R_macro,
                            final_result_list_F_macro,
                            final_result_list_P_micro, final_result_list_R_micro, final_result_list_F_micro,
                            each_fold_average_list, eval(args.Task_list), "Bert")
